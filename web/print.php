...
$regs = 0;
$records = array();
if(isset($_GET['date']) && isset($_GET['cluster']) && isset($_GET['host']) && isset($_GET['vmname']) && isset($_GET['folder']) && isset($_GET['vmstate'])&&isset($_GET['ip_primary'])&&isset($_GET['vmwtools'])&&isset($_GET['annotation'])&&isset($_GET['sortby'])&&isset($_GET['order']))
{
	...
	$query = "select datetime,vmname,cluster,host,folder,ip_primary,provisioned_space,guest_disk_usage,usage_storage,path,mem,cpu,sockets,ostype,state,annotation,boottime,max_cpu_usage,max_memory_usage FROM vminfo WHERE datetime LIKE '" . $date ."%'"  ;
	...
	if ($num>0)
	{
		...
		if ($host!='0')	$query = $query . "host='$host' AND ";	
...
if (!isset($_GET["export"])){
	...
	echo '<th>'.'boottime'.'</th>';
	echo '<th>'.'sockets'.'</th>';
	echo '<th>'.'max_cpu_usage'.'</th>';
	echo '<th>'.'max_memory_usage'.'</th>';
	echo '<th>'.'path'.'</th>';
       ...
       $vms[$i] = mysqli_fetch_assoc($regs); 
		...
		{			
			if (!isset($_GET["export"])){
				...
				echo "<td>".$vms[$i]['boottime']."</td>\n";
				echo "<td>".$vms[$i]['sockets']."</td>\n";
				echo "<td>".$vms[$i]['max_cpu_usage']."</td>\n";
				echo "<td>".$vms[$i]['max_memory_usage']."</td>\n";
